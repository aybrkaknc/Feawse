...
import static com.example.feawse.data.ItemDb.*;
...
public class TranBlock {
    ...
    public void cleanInventory(List<Refinement> refiList) {
        List<Integer> usedRefiPositions = new ArrayList<>();
        if (refiList != null) {
            for (Refinement refinement : refiList) {
                usedRefiPositions.add(refinement.position());
            }
        }
        for (int i = 0; i < inventoryRefi.size(); i++) {
            if (!usedRefiPositions.contains(i)) {
                inventoryRefi.set(i, 0);
            }
        }
    }
