package com.example.feawse.savefile.units;

import com.example.feawse.data.SkillDb;
import com.example.feawse.data.model.SkillModel;
import com.example.feawse.savefile.units.extrablock.ChildBlock;
import com.example.feawse.savefile.units.extrablock.LogBlock;
import com.example.feawse.savefile.units.mainblock.*;
import com.example.feawse.savefile.wireless.UnitDu;
import com.example.feawse.util.Hex;

import java.io.ByteArrayOutputStream;
import java.util.Arrays;
import java.util.List;

import static com.example.feawse.data.ClassDb.*;
import static com.example.feawse.data.UnitDb.*;
...
public class Unit {
    ...
    public boolean isDead() {
        return rawBlockEnd.deadFlag1() || rawBlockEnd.deadFlag2() || rawFlags.hasBattleFlag(3) || rawFlags.hasBattleFlag(7);
    }
    ...
    public UnitDu toUnitDu(boolean isWest) {
        ...
        //Items
        if (rawInventory != null && rawInventory.items != null) {
            for (int i = 0; i < 5; i++) {
                if (i < rawInventory.items.size()) {
                    int itemId = rawInventory.items.get(i).itemId();
                    if (itemId > 0 && i < unitDu.itemList.size()) {
