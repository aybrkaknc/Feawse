package com.example.feawse.savefile.inventory;
...
public class RefiBlock {
    ...
    public boolean addRefinement(Refinement refinement) {
        if (refiList.size() >= TranBlock.MAX_FORGED) return false;
        if (!refiList.contains(refinement)) {
            refiList.add(refinement);
        }
        return true;
    }

    public boolean removeRefinement(Refinement refinement) {
        return refiList.remove(refinement);
    }

    public boolean removeRefinement(int position) {
        return refiList.removeIf(r -> r.position() == position);
    }
}
