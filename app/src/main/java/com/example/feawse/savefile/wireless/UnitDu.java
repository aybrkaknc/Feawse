package com.example.feawse.savefile.wireless;
...
import com.example.feawse.savefile.units.mainblock.RawSkill;
import com.example.feawse.util.Hex;
import com.example.feawse.util.Color;
...
import static com.example.feawse.util.Hex.*;
...
public class UnitDu {
    ...
    public byte[] bytesFull() {
        ...
        try {
            ...
            else byteArrayOutputStream.write(new byte[5]);
            byteArrayOutputStream.write(rawBlock1);
            for (DuItem item : itemList) byteArrayOutputStream.write(item.bytes());
            byteArrayOutputStream.write(rawChild);
            byteArrayOutputStream.write(rawSkill.bytes());
